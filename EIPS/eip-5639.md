...
author: foobar (@0xfoobar), Wilkins Chung (@wwhchung), ryley-o (@ryley-o), Jake Rockland (@jakerockland), andy8052 (@andy8052), Lukas Schor (@lukasschor)
...
Many dApps require the user to prove control of a wallet to gain access. At the moment, this means that you must interact with the dApp using the wallet itself. This is a security issue, as malicious dApps or phishing sites can lead to the assets of the wallet being compromised by having them sign malicious payloads.

This risk is mitigated if the user can use a secondary wallet for these interactions. Malicious interactions are isolated to the assets held in the secondary wallet, which can be set up to contain little to nothing of value.
...
Many individuals use hardware wallets for maximum security. However, this is often inconvenient, since many do not want to carry their hardware wallet with them at all times.
...
A dApp or smart contract would check whether or not a delegate is authenticated for a token contract by checking the return value of checkDelegateForContract.

A dApp or smart contract would check whether or not a delegate is authenticated for a specific token owned by a vault by checking the return value of checkDelegateForToken.

A delegate can act on a specific token if they have a token level delegation, contract level delegation (for that token's contract) or vault level delegation.

A delegate can act on all tokens of a given contract if they have contract level delegation or vault level delegation.

A delegate can act on tokens from any token contract if they have vault level delegation.
